...
public:
    filtered_simplex (const filtered_simplex &s) {index = s.index; filtration_value = s.filtration_value;}
    filtered_simplex (index_t ind, value_t fval) {index = ind;  filtration_value=fval;}
    filtered_simplex(index_t ind){index = ind; filtration_value = 0;}
...
class filtered_simplex_coeff{
    ...
    value_t filtration_value;
public:
    filtered_simplex_coeff (const filtered_simplex_coeff &s) {index = s.index; coefficient = s.coefficient; filtration_value = s.filtration_value;}
    filtered_simplex_coeff (index_t ind, coefficient_t coeff, value_t fval) {index = ind; coefficient = coeff; filtration_value=fval;}
    filtered_simplex_coeff (index_t ind){index = ind; coefficient = 1; filtration_value = 0;}
    filtered_simplex_coeff (filtered_simplex s, coefficient_t coeff){this->index = s.get_index(); this->filtration_value = s.get_filtration_value();
...
bool next_face(const Iterator first, Iterator k, const Iterator last);
...
public:
    ...
    void assign_highest_facet(std::unordered_map<index_t, simplex> *simplicialFiltration, const binomial_coeff_table& B){
    ...
    void assign_lowest_cofacet(std::unordered_map<index_t, simplex> *simplicialFiltration, const binomial_coeff_table& B, index_t n){
        ...
        for (index_t i = 0; i < n; ++i) {
            if (!(std::binary_search (vertices.begin(), vertices.end(), i, descend_compare))){
                ...
                if (simplicialFiltration[cofacet_dim].find(index) != simplicialFiltration[cofacet_dim].end()) {
                    std::unordered_map<index_t, simplex>::const_iterator S = simplicialFiltration[cofacet_dim].find(index);
                    value_t fval = S->second.get_filtration();
    ...
    void sort(){
        std::sort(this->vertices.begin(), this->vertices.end(), descend_compare);
    }
...
void assemble_columns_to_reduce2(std::vector<filtered_simplex>& columns_to_reduce,
                                hash_map<index_t, index_t>& pivot_column_index,  std::unordered_map<index_t, simplex> *simplicialFiltration, index_t dim,
    ...
    for (index_t index = 0; index < num_simplices; ++index) {
        if (pivot_column_index.find(index) == pivot_column_index.end() && simplicialFiltration[dim+1].find(index)!=simplicialFiltration[dim+1].end()) {
...
void compute_pairs2(std::vector<filtered_simplex>& columns_to_reduce, hash_map<index_t, index_t>& pivot_column_index,
                   ...
                   const std::vector<coefficient_t>& multiplicative_inverse, std::unordered_map<index_t, simplex> *simplicialFiltration, const binomial_coeff_table& binomial_coeff) {
    ...
    for (index_t i = 0; i < columns_to_reduce.size(); ++i) {
...
#ifdef ASSEMBLE_REDUCTION_MATRIX
        std::priority_queue<filtered_simplex_coeff, std::vector<filtered_simplex_coeff>, smaller_index2<filtered_simplex_coeff>>
        reduction_column;
#endif
        ...
        if ((i + 1) % 1000 == 0)
            std::cout << "\033[K"
            << "reducing column " << i + 1 << "/" << columns_to_reduce.size() << " (Filtration value:  " << filtration_val
            << ")" << std::flush << "\r";
#endif
            ...
            for (auto it = coeffs_begin; it != coeffs_end; ++it) {
                ...
                while (cofaces.has_next()) {
                    ...
                    if (simplicialFiltration[dim+1].find(ind)!=simplicialFiltration[dim+1].end()) {
                        std::unordered_map<index_t, simplex>::const_iterator S = simplicialFiltration[dim+1].find(ind);
                        value_t fval = S->second.get_filtration();
                        coface.set_filtration(fval);
                        if (fval <= threshold) {
                            coface_entries.push_back(coface);
                            if (might_be_apparent_pair && (current_simplex.get_filtration_value() == coface.get_filtration_value())) {
...
void missing_faces_helper_routine(std::unordered_map<index_t, simplex> *simplicialFiltration, std::vector<index_t> simplex_vertices, const binomial_coeff_table& B, index_t n, index_t face_dim){
    simplex face;
    face.clear();
...
void add_missing_faces(std::unordered_map<index_t, simplex> *simplicialFiltration, index_t dim_max, index_t n, const binomial_coeff_table& B){
  /*
  ...
  */
    ...
    for (index_t dim = 1; dim <= dim_max; ++dim){
        for ( auto simplicialIterator = simplicialFiltration[dim].begin(); simplicialIterator != simplicialFiltration[dim].end(); ++simplicialIterator ){
            std::vector<index_t>::iterator a, b;
         ...
         /*   for ( auto i = simplex_vertices.begin(); i != simplex_vertices.end() ; ++i)
            ...
            for (index_t face_dim = 0; face_dim < dim; ++face_dim) {
                ...
                } while (next_face(simplex_vertices.begin(), simplex_vertices.begin() + face_dim + 1, simplex_vertices.end()));
            }
        }
    }
...
int main(int argc, const char * argv[]) {
    ...
    
    {
        ...
        for (auto e : edges) {
            ...
            if (u != v) {
                ...
                if (e.get_filtration_value() > 0) std::cout << " [" << paired_fval << "," << e.get_filtration_value() << ")" << std::endl;
#endif
                dset.link(u, v);
            } else
    ...
    for (index_t dim = 1; dim < dim_max; ++dim) {
        ...
        compute_pairs2(columns_to_reduce, pivot_column_index, dim, n, threshold, modulus, multiplicative_inverse, simplicialFiltration, binomial_coeff);
        
        if (dim < dim_max-1) {
            assemble_columns_to_reduce2(columns_to_reduce, pivot_column_index, simplicialFiltration, dim, n, threshold, binomial_coeff);
        }
    }
